...
const SidebarProvider = React.forwardRef(function SidebarProvider(
  ...
  const setOpen = React.useCallback(
    (v) => {
      const newState = typeof v === "function" ? v(open) : v
...
const SidebarRail = React.forwardRef(function SidebarRail({ className, ...props }, ref) {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:@" + 
        "w-[2px] hover:after:bg-sidebar-border group-data-[side=left]:-right-4 group-data-[side=right]:left-0 sm:flex",
...
const SidebarGroupAction = React.forwardRef(function SidebarGroupAction({ className, asChild, showOnHover, ...props }, ref) {
...
const SidebarMenuButton = React.forwardRef(function SidebarMenuButton({ asChild, isActive, tooltip, variant, size, className, ...props }, ref) {
  const Comp = asChild ? Slot : "button"
